...
- The **published JSON Schema constrains shape, not semantics.** Measured: **10 of 12** deliberately wrong-but-well-formed artifacts validate against it, while **both shipped readers reject all 12**. So no wrong number reaches you *through this library* — but the schema's `$id` is a public URL, and if you validate against it and then walk the artifact with your own code, it is weaker than the word "schema" suggests. The catalogue of 12 is committed and the accepted set is pinned exactly, so the gap cannot widen unnoticed, and the schema's own `description` now states its scope. Earlier releases of this document cited "ten of eleven" from an audit's prose with no committed artifact behind it; that figure was not reproducible from a clone, which this file promises.
